...
from view.main_panel import reformat_study_name, reverse_reformat
...
def select_study_view(main_holder: st.empty):
    ...
    with main_holder.container():
        ...
        with col2:
            ...
            selected_category = st.selectbox("Category:", [reformat_study_name(d) for d in listdir(language_path) if isdir(join(language_path, d))])
            category_path = join(language_path, reverse_reformat(selected_category))
            # After applying filters, show which study can be selected.
            selected_study = st.selectbox("Please select what you would like to study:", [reformat_study_name(d) for d in listdir(category_path) if isdir(join(category_path, d))])
    ...
    if select:
        ...
        st.session_state["study_path"] = join(category_path, reverse_reformat(selected_study))
